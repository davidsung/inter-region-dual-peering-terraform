...
resource "aws_route" "peer_route_in_local" {
  provider                  = aws.local
  count                     = length(var.peer_private_cidr_blocks)
  route_table_id            = var.local_route_table_id
  destination_cidr_block    = var.peer_private_cidr_blocks[count.index]
  vpc_peering_connection_id = aws_vpc_peering_connection.vpc_peering_connection.id
...
resource "aws_route" "local_route_in_peer" {
  provider                  = aws.peer
  count                     = length(var.local_private_cidr_blocks)
  route_table_id            = var.peer_route_table_id
  destination_cidr_block    = var.local_private_cidr_blocks[count.index]
  vpc_peering_connection_id = aws_vpc_peering_connection.vpc_peering_connection.id
  depends_on                = [aws_vpc_peering_connection_accepter.vpc_peering_connection_accepter]
}
